...
//                (sig1, sig2) -> Integer.compare(sig1.algorithm.getId(), sig2.algorithm.getId()));
		Collections.sort(signaturesToVerify, new Comparator<ApkSupportedSignature>() {
				@Override
				public int compare(ApkSupportedSignature p1, ApkSupportedSignature p2) {
					return Integer.compare(p1.algorithm.getId(), p2.algorithm.getId());
				}
			});
